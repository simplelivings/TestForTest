 ...
 * required forms.
 * 所有暴露的方法都是包私有的，设计成为从class数组的公共方法中引用，这些公共方法已经进行了必要的数组边界检查，
 * 并且将参数展开为需要的形式。
 *
...
final class DualPivotQuicksort {
     ...
     * 如果被排序数组的长度小于这个常量，会优先用快速排序去合并排序
     */
     ...
     * 如果被排序数组中的长度小于这个常量，会优先用插入排序去合并排序
     */
     ...
     * 如果被排序字节数组的长度大于这个常量，计数排序会优先用于插入排序中；
     */
     ...
     * 如果被排序的短数组或字节数组的长度大于这个常量，计数排序会优先用于快速排序中
     */
     ...
     * 如果可以合并的话，使用给定的工作区数组切片，对数组的指定范围排序
     *
     * @param a the array to be sorted  待排序的数组
     * @param left the index of the first element, inclusive, to be sorted
     *             待排序数组的第一个元素的索引，包含第一个元素
     * @param right the index of the last element, inclusive, to be sorted
     *              待排序数组的最后一个元素的索引，包含最后一个元素
    ...
    static void sort(int[] a, int left, int right,
        ...
        // 实施说明：变量right从1开始增加；
        if (run[count] == right++) { // The last run contains one element run数组的最后一个元素有值
            run[++count] = right;
        } else if (count == 1) { // The array is already sorted 数组已经被排序
            return;
        }

        // Determine alternation base for merge 决定合并排序的替代基准
        byte odd = 0;
        for (int n = 1; (n <<= 1) < count; odd ^= 1);

        // Use or create temporary array b for merging
        // 使用或创建一个临时数组b，用于合并排序
        int[] b;                 // temp array; alternates with a 临时数组，和数组a交替
        int ao, bo;              // array offsets from 'left' 数组的左偏移量
        int blen = right - left; // space needed for b b需要的空间
    ...
    private static void sort(int[] a, int left, int right, boolean leftmost) {
        ...
        // Use insertion sort on tiny arrays 微型数组，使用插入排序
        if (length < INSERTION_SORT_THRESHOLD) {
            if (leftmost) {
                /*
                 * Traditional (without sentinel) insertion sort,
                 * 无哨兵的传统插入分类，为服务器VM优化，用于最左边的部分
                 ...
                 * of sentinel,
                 *  therefore this allows us to avoid the
                 * left range check on each iteration.
                 * 来自连接部分的每个元素，都扮演哨兵的作用；以避免对每一次迭代进行左侧检查；
                 *
                 * Moreover, we use
                 * the more optimized algorithm, so called pair insertion
                 * sort, which is faster (in the context of Quicksort)
                 * than traditional implementation of insertion sort.
                 * 此外，我们使用更加优化的算法，叫做成对插入分类，这比传统的插入算法更快（在快速排序的情况下）
                 */
        ...
        // Inexpensive approximation of length / 7 简单近似7的倍数
